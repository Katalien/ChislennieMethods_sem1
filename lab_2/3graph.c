...
int main() {
	...
	int start = 3;
	int n = 9;
	char* matrixFile = "C:/Users/z.kate/Desktop/3 сем/численные методы/3matrix.csv";
	char* vectorFile = "C:/Users/z.kate/Desktop/3 сем/численные методы/3vector.csv";
	char* xFile = "C:/Users/z.kate/Desktop/3 сем/численные методы/time.csv";
	...
	for (int k = 0; k < 100; k++) {
		...
		double tmp = (end.QuadPart - begin.QuadPart) / (double)freq.QuadPart;
		fprintf(fp3, "%.8lf ", tmp );
		free(L);
		free(U);
		free(x);
